...
class PracticeBox extends Component {
	...
	handleSubmit = (e) => {
		...
		const allExcerpts = [];
		const goldExcerpts = this.props.goldExcerpts;
		if (goldExcerpts) {
		...
		const goldCodes = this.props.goldCodes;
		let selected = false;
		this.state.codes.forEach((code, index) => {
			if (index > 0 && index < this.props.codes.length) {
				if (code) {
					// learner selected
					selected = true;
					if (goldCodes && goldCodes.includes(index.toString())) {
						correctRows.push({
		...
		console.log(goldCodes)
		console.log(this.state.codes)
		if (!selected && !goldCodes) {
			// the learner selected no codes and there were none to select
			console.log('it is being triggered')
			correctRows.push({text:"You correctly identified that there are no codes for this case.", code:''});
		}
